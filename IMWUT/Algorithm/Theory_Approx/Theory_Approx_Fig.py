...
for index_led, led in led_df.iterrows():
    ...
    for index, row in exp_plot_df.iterrows():
        ...
        ax.scatter(list(map(int, exp_plot_df.columns.tolist())), row.tolist(), marker='x', color='red', label ='Experimental')
    ...
    #Scaled Theory
    ax.scatter(list(map(int, exp_plot_df.columns.tolist())), [x*sf+bias for x in areas], marker='*', color='green', label ='Theoretical')
    ...
    print('Pearson: ',pearsonr([x*sf+bias for x in areas],row.tolist()))
